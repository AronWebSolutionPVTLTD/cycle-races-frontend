...
// Global home page search - multi search for riders, races, teams
export const homePageSearch = (searchQuery = "") => {
  ...
  return API.get(`/stats/HomePageSearch${queryParams}`)
    .then((res) => {
      // API response structure: { status: true, message: "...", data: { results: [...] } }
      let rawData = [];
      ...
      if (Array.isArray(res.data)) {
        ...
        // Nested structure: data.data.results
        rawData = res.data.data.results;
      } else if (res.data?.results && Array.isArray(res.data.results)) {
        // Alternative nested structure: data.results
...
export const fetchData = async (
  ...
  try {
    ...
    if (endpointsMappings[endpointKey]) {
      ...
      // 3. If we have a name, use the name-based endpoint format for race-specific stats
      endpointPath = `/${idType}/${name}/${endpointKey}`;
    } else {
      // 4. Fallback to standard endpoint from the endpoints object
      endpointPath = validateEndpoint(endpointKey);
    }
    // endpointPath = endpointsMappings[endpointKey]
    //   ? endpointsMappings[endpointKey]
    //   : endpointKey;
...
export const fetchMultiple = async (endpointKeys, options = {}) => {
  ...
  const promises = endpointKeys.map((key) => {
    try {
      ...
      if (endpointsMappings[key]) {
        // 1. If we have a custom endpoint mapping for this key
        // The endpoint mapping should already have encoded values if needed
        endpointPath = id
